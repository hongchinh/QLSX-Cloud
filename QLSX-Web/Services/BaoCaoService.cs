...
{
	interface IBaoCaoService<T>
	{
		Task<List<T>> GetAllAsync();
		Task<List<T>> GetAllAsync(QLSX.Shared.Models.BaoCaoRequest request);
		Task<T> GetByIdAsync(int Id);
		Task<T> CreateAsync(T obj);
		Task<T> UpdateAsync(int Id, T obj);
		Task<bool> DeleteAsync(int Id);
		Task<string> GetPathReportAsync(QLSX.Shared.Models.BaoCaoRequest request);
		Task ClearDataInAsync();


	}

	public class BaoCaoService<T> : IBaoCaoService<T>
	{
		public HttpClient _httpClient { get; }
		public AppSettings _appSettings { get; }
		public ILocalStorageService _localStorageService { get; }
		private string requestUri = "baocaos";
		public AppService _appService { get; }
		public BaoCaoService(HttpClient httpClient
		...
		public async Task<string> GetPathReportAsync(QLSX.Shared.Models.BaoCaoRequest request)
		{
			var token = await _localStorageService.GetItemAsync<string>("accessToken");
			var mdvsd = _appService.DMDonViSuDungId;
			return _appSettings.ReportUrl + "Reports/Viewer?ReportFileName=" + request.FileName +
				"&type=" + request.Type +
				"&tungay=" + string.Format("{0:yyy/MM/dd}", request.TuNgay) +
				"&denngay=" + string.Format("{0:yyy/MM/dd}", request.DenNgay) +
				"&token=" + token +
				"&id=" + mdvsd +
				"&mdvsd=" + mdvsd;
		}
		...
		public async Task<List<T>> GetAllAsync(QLSX.Shared.Models.BaoCaoRequest request)
		{
		...
		public async Task ClearDataInAsync()
		{
			try
			{
				var requestMessage = new HttpRequestMessage(HttpMethod.Post, "BaoCaos/ClearListPrint");
				var token = await _localStorageService.GetItemAsync<string>("accessToken");
				requestMessage.Headers.Authorization
					= new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
				var response = await _httpClient.SendAsync(requestMessage);
